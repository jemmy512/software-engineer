...
public:
    ...
    void checkIn(int id, string stationName, int t) {
        _CheckInInfo.emplace(id, std::pair{stationName, t}); // if data exists, nothing happen
        // _CheckInInfo[id] = std::make_pair(stationName, t);
    }

    void checkOut(int id, string stationName, int t) {
        const auto& [startStation, startTime] = _CheckInInfo[id];
        const auto& key = stationKey(startStation, stationName);
        auto& [total, size] = _StationInfo[key];
        total += t - startTime;
        size += 1;
        _CheckInInfo.erase(id);
    }

    double getAverageTime(string startStation, string endStation) {
        const auto& key = stationKey(startStation, endStation);
        const auto& [total, size] = _StationInfo[key];
        return static_cast<double>(total) / size;
    }

private:
    string stationKey(const string& start, const string& end) {
        return start + "-" + end;
    }

private:
    std::unordered_map<int, std::pair<string, int>> _CheckInInfo; // <userId, <startStation, time>>
    std::unordered_map<string, std::pair<int, int>> _StationInfo; // <stationKey, <total, size>>
    // unordered_map<string,unordered_map<string,pair<int,int>>> _StationInfo; // <start, <endStation, <total, size>>>
};
