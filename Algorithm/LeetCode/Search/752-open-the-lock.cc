...
public:
    int openLock(vector<string>& deadends, string target) {
        int steps = 0;
        queue<string> que;
        set<string> visited(deadends.cbegin(), deadends.cend());
        ...
        while (!que.empty()) {
            ...
            for (auto i = 0; i < size; ++i) {
                ...
                // const auto& code = std::move(que.front());
                const auto code = std::move(que.front());
                que.pop();
                ...
                for (auto j = 0; j < 4; ++j) {
                    const auto plus = plusOne(code, j);
                    if (visited.find(plus) == visited.end()) {
                        que.push(plus);
                        visited.emplace(plus);
                    }

                    const auto minus = minusOne(code, j);
                    if (visited.find(minus) == visited.end()) {
                        que.push(minus);
                        visited.emplace(minus);
                    }
                }
            }
...
private:
    string plusOne(string code, int i) {
        code[i]  = (code[i] == '9') ? '0' : code[i] += 1;
        return std::move(code);
    }

    string minusOne(string code, int i) {
